...
// ─── The agent's MCP tools: what reply / react / no_reply / edit_message do ────────
// Written once against `Chat`. Each call that answers a message sends a `Disposition` to the
// main loop, which clears the ledger in `on_disposition` above.

/// Send a disposition to main's ledger. Waits if full (dropping it would leave the ledger entry behind).
async fn notify(
    dispo: &tokio::sync::mpsc::Sender<Disposition>,
    d: Disposition,
    ctx: &LogCtx,
) {
...
pub struct ToolExec {
    ...
    pub dispo: tokio::sync::mpsc::Sender<Disposition>,
}
...
pub async fn post_chunked(
    ...
    let a = bridge::Access::load(&crate::state_dir::StateDir::at(state_dir));
    let limit = a
        .text_chunk_limit
        .unwrap_or(slack::MAX_CHUNK_LIMIT)
        .clamp(1, slack::MAX_CHUNK_LIMIT);
    ...
    for part in slack::chunk(text, limit, a.chunk_mode.as_deref() == Some("newline")) {
        let ts = if markdown {
...
pub async fn execute_tool(
    ...
    let out = match tool {
        "reply" => {
            ...
            let posted = match check_attachment_sizes(&files, slack::MAX_ATTACHMENT_BYTES) {
                Err(e) => Err(e),
        ...
        "download_attachment" => slack::download_attachment(slack, &s("file_id"), state_dir).await,
        "no_reply" => {
...
mod tests {
    ...
    fn harness() -> (
        FakeChat,
        std::path::PathBuf,
        tokio::sync::mpsc::Sender<Disposition>,
        tokio::sync::mpsc::Receiver<Disposition>,
    ) {
        let (tx, rx) = tokio::sync::mpsc::channel(8);
        // Assumes the root is alive (suppression for deleted threads has its own test)
        (
            FakeChat::with_live_root(),
    ...
    fn oversized_or_missing_attachment_is_rejected() {
        ...
        let ok = check_attachment_sizes(&paths, slack::MAX_ATTACHMENT_BYTES);
